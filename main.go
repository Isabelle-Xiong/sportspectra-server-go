...
var expire uint32 = 3600

func getRtmToken(c *gin.Context) {

	log.Printf("rtm token\n")
	// get param values
	uidStr, expire, err := parseRtmParams(c)
	...
	rtmToken, tokenErr := rtmtokenbuilder2.BuildToken(appID, appCertificate, uidStr, expire, "newStream")
...
func getBothTokens(c *gin.Context) {
	...
	// generate rtmToken
	rtmToken, rtmTokenErr := rtmtokenbuilder2.BuildToken(appID, appCertificate, uidStr, expire, "newStream")
...
func parseRtmParams(c *gin.Context) (uidStr string, expire uint32, err error) {
	// get param values
	uidStr = c.Param("uid")
	expireTime := c.DefaultQuery("expiry", "3600")
	...
	expireTimeInSeconds := uint32(expireTime64)
	currentTimestamp := uint32(time.Now().UTC().Unix())
	expire = currentTimestamp + expireTimeInSeconds
